...
import { useAdminResendOtp } from '@/hooks/useAdminResendOtp';
...
function AdminVerifyOTPContent() {
  ...
  const handleVerify = async (otpCode: string) => {
    ...
    try {
      await adminAuthService.verifyOtp({ email, otp: otpCode });
      router.push(`/admin/reset-password?email=${encodeURIComponent(email)}&token=${otpCode}`);
    } catch (err) {
  ...
  return (
    <AuthLayout>
      ...
      <form onSubmit={handleSubmit} className="mt-12 space-y-8">
        ...
        <AuthBackLink href="/admin/forgot-password" label="Back" />
      </form>
