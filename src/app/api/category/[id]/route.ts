...
export const GET = withDb(async (
  ...
  try {
    ...
    const category = await Category.findById(id).populate('superCategories');
...
export const PUT = withAdmin(async (
  ...
  try {
    ...
    if (image !== undefined) {
      updateData.image = await uploadImageIfNeeded(image, 'categories');
    }
      ...
      if (duplicate) {
        return NextResponse.json({
          ...
          message: duplicate.name === updateData.name
            ...
            : CATEGORY_MESSAGES.DUPLICATE_SLUG,
          statusCode: 400
        }, { status: 400 });
      }
    }
