...
export const GET = withAdmin(async (_request: NextRequest, _user: IUser, context: RouteContext) => {
  try {
    const { slug } = await context.params;
    const page = await CmsPage.findOne({ slug: slug.toLowerCase() }).lean();
    if (!page) {
      return NextResponse.json({ status: false, message: 'Page not found', statusCode: 404 }, { status: 404 });
    }
...
export const DELETE = withAdmin(async (_request: NextRequest, _user: IUser, context: RouteContext) => {
  try {
    ...
    return NextResponse.json({ status: true, message: 'Page deleted', statusCode: 200 });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Internal server error';
    return NextResponse.json({ status: false, message, statusCode: 500 }, { status: 500 });
  }
