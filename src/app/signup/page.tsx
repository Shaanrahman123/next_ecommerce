...
export default function SignupPage() {
    ...
    const handleSocialSignup = async (provider: 'google' | 'facebook' | 'apple') => {
        ...
        try {
            // Simulated OAuth provider details
            ...
            if (!response.ok || !data.status) {
                setApiError(data.message || `${provider} registration failed.`);
                setIsLoading(false);
                return;
            }

            // Successfully registered/logged in via social auth
            signup({
                id: data.user.id,
                email: data.user.email,
                name: `${data.user.firstName} ${data.user.lastName}`,
            ...
            setApiError(err.message || `An error occurred during ${provider} registration.`);
            setIsLoading(false);
        }
    };
