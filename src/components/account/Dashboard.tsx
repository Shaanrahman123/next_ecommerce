...
export default function Dashboard() {
    ...
    const dashboardItems = [
        ...
        { label: 'Update Profile', icon: UserCircle, color: 'purple', link: '/account?section=profile' },
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="hidden lg:block">
                <h1 className="text-section-title font-black text-heading mb-2 uppercase tracking-tight">Dashboard</h1>
                    ...
                    return (
                        ...
                        >
                            <div className="flex flex-col items-center lg:flex-row lg:items-center gap-3 lg:gap-5">
                                <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-md bg-gray-50 flex items-center justify-center transition-all duration-500 group-hover:rotate-6 group-hover:bg-primary group-hover:text-white shrink-0`}>
                                    <Icon className={`w-6 h-6 text-gray-900 group-hover:text-white transition-colors`} />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] lg:text-body font-black text-heading uppercase tracking-tight break-all">{item.label}</p>
                            ...
                            <ChevronRight className="hidden lg:block w-5 h-5 text-gray-300 group-hover:text-heading transition-colors" />
                        </Link>
                    );
                })}
            </div>
            ...
            <div className="pt-4 lg:pt-0">
                <div className="flex items-center justify-between mb-6 lg:mb-8">
                    <h2 className="text-body font-black text-heading uppercase tracking-widest border-l-4 border-primary pl-3">Recent Orders</h2>
                    <Link
                        href="/account?section=orders"
                        className="text-[10px] font-black text-heading hover:underline uppercase tracking-widest"
                    >
                        View All
                    </Link>
                </div>
                    ...
                    {recentOrders.map((order) => (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-white rounded-md flex items-center justify-center group-hover:bg-primary transition-colors duration-500">
                                    <ShoppingBag className="w-6 h-6 text-heading group-hover:text-white" />
                                </div>
                                <div>
                                    <p className="text-small font-black text-heading uppercase tracking-tighter">Order {order.id}</p>
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{order.items} items • {order.date}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <p className="text-small font-black text-heading mb-1">{order.amount}</p>
